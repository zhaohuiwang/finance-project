...
class SymbolConfig(BaseModel):
    ...
    fixed_shares: int = Field(
        default=100, ge=1, description="Number of shares to trade per order"
    )
    ...
    trailing_sell_pct: float = Field(
        default=2.0, ge=0.1, description="a% trailing stop percentage for sell orders"
    )
    ...
    trailing_buy_pct: float = Field(
        default=1.5, ge=0.1, description="b% trailing stop percentage for buy orders"
    )

    # Additional risk controls
    max_position_value: float = Field(
        default=10000.0, ge=0.0, description="Maximum dollar value per position"
    )
...
class RiskConfig(BaseModel):
    ...
    max_positions: int = Field(
        default=4, ge=1, description="Maximum number of concurrent positions"
    )
...
class TradingConfig(BaseModel):
    ...
    auto_shutdown_after_close: bool = Field(
        default=True, description="Automatically shutdown after market close"
    )
    ...
    shutdown_on_weekends: bool = Field(default=True, description="Shutdown on weekends")
...
DEFAULT_CONFIG = {
    "risk": {"min_account_equity": 5000, "max_positions": 4},
    "symbols": {
        "AAPL": {
            "fixed_shares": 50,
            "momentum_up_pct": 5.0,
