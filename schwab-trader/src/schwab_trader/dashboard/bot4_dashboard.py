...
def run_dashboard(bot, port=8050):
        ...
        [
            dbc.Row(
                [
                    dbc.Col(
                        html.H1(
                            "Schwab Trailing Momentum Bot", className="text-center mb-4"
                        ),
                        width=12,
                    )
                ]
                ...
                [
                    dbc.Col(
                        [
                            ...
                            dcc.Interval(
                                id="interval", interval=5000, n_intervals=0
                            ),  # 5s refresh
                        ],
                        width=8,
                    ),
    ...
    def update_dashboard(n, refresh_clicks):
        ...
        prices_table = dbc.Table(
            [
                html.Thead(
                    html.Tr([html.Th("Symbol"), html.Th("Price"), html.Th("Momentum")])
                )
            ]
        ...
        holdings_table = (
            dbc.Table(
                [
                    html.Thead(
                        html.Tr(
                            [html.Th("Symbol"), html.Th("Shares"), html.Th("Avg Price")]
                        )
                    )
                ]
    ...
    @app.callback(Output("status", "children"), Input("reload-btn", "n_clicks"))
    ...
    @app.callback(Output("status", "children"), Input("stop-btn", "n_clicks"))
    def stop_bot_callback(n):
        if n:
            bot.stop()
            return dbc.Alert(
                "Bot stopped. Restart the script to run again.", color="danger"
            )
        return dash.no_update
